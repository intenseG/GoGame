...
public class GoUtil : MonoBehaviour
{
    ...
    public static string FormatFinalScore(string result)
    {
        ...
        while (true)
        {
            if (result.Length > 1 && stateNum != (int) GoEstimator.RequestState.Empty) break;
            count++;
            if (count > 10000)
            {
    ...
    public static string GetWinnerColor(string resultStr, int stateNum)
    {
        ...
        while (true)
        {
            if (resultStr.Length > 1 && stateNum != (int) GoEstimator.RequestState.Empty) break;
            count++;
            if (count > 10000)
            {
                //Debug.Log("強制的にループを抜けます");
